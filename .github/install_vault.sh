...
#  fmt "\tlogin        \tlogin to vault using the root token"
#  fmt "\techoinAsUser  \tlogin to vault using parameters: <user> <password>. Default: bob/sinclair"
...
function login() {
  echo "login in as Root"
  ROOT_TOKEN=$(jq -r ".root_token" ${TMP_DIR}/cluster-keys.json)
  vaultExec "vault login ${ROOT_TOKEN}"
}

function rootToken() {
  echo"" "Vault root token: $(kubectl get secret -n ${VAULT_NAMESPACE} tokens -ojson | jq -r '.data.root_token' | base64 -d)"
}
...
function enableKV1SecretEngine() {
  echo "Enable the KV version 1 secret engine. The secrets engine will be mounted to the path: ${KV1_PREFIX}"
  vaultExec "vault secrets enable -path=${KV1_PREFIX} -version=1 kv"
}

function enableKV2SecretEngine() {
  echo "Enable KV version 2 secret engine. The secrets engine will be mounted to the path: ${KV2_PREFIX}"
  vaultExec "vault secrets enable -path=${KV2_PREFIX} -version=2 kv"
}

function enableK8sSecretEngine() {
  echo "Enable Kubernetes secret engine"
  vaultExec "vault secrets enable kubernetes"
}

function enableUserPasswordAuth() {
  echo "Enable User/password auth"
  vaultExec "vault auth enable userpass"
}

function createTokensKubernetesSecret() {
  echo "Creating a kubernetes secret storing the Vault Root Token"
...
function createUserPolicy() {
  ...
  echo "Creating policy ${VAULT_POLICY_NAME} for path: ${KV1_PREFIX}/${KV_APP_NAME}/* having as roles: ${ROLES}"
...
function registerUser() {
  ...
  echo "Creating a new vault user: ${VAULT_USER}, password: ${VAULT_PASSWORD} having as policy: ${VAULT_POLICY_NAME}"
...
# echo "Put the key hello = world at the mounted path: ${KV1_PREFIX}/${KV_APP_NAME}"
#vaultExec "vault kv put ${KV1_PREFIX}/${KV_APP_NAME}/hello target=world"

echo "Executing: vault kv put -mount=${KV2_PREFIX} ${KV_APP_NAME}/hello target=world"
vaultExec "vault kv put -mount=${KV2_PREFIX} ${KV_APP_NAME}/hello target=world"

echo "Vault temp folder containing the generated files: ${SCRIPTS_DIR}/../${TMP_DIR}"
echo "Vault Root Token: $(jq -r ".root_token" ${TMP_DIR}/cluster-keys.json)"

echo "Vault Root Token can be found from the kubernetes secret: \"kubectl get secret -n ${VAULT_NAMESPACE} tokens -ojson | jq -r '.data.root_token' | base64 -d\""